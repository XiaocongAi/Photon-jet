...
def main():
    ...
    sigEff.SetTitle("%s;E_{H} [GeV];Efficiency"%("Unbinned training")); 
    bkg0Eff.SetTitle(";E_{H} [GeV];"); 
    
    sigEff_binned.SetTitle("%s;E_{H} [GeV];Efficiency"%("Binned training")); 
    bkg0Eff_binned.SetTitle(";E_{H} [GeV];"); 
...
#    X_test_bkg1_ene_binned = [[], [], [], [], [], [], [], [], [], []]
    ...
    # IMPORTANT: The dimension of decisions_train is equal to the number of labels (one-vs-all for each label)
    ...
    for i in range (len(X_train_binned)):
        ...
        compare_train_test(bdt_, processLabels, iColForSig, np.array(X_train_binned[i]), np.array(y_train_binned[i]), np.array(X_test_binned[i]), np.array(y_test_binned[i]), output, filepath, "_bin%i"%(i), 50, sigLegend, bg0Legend, bg1Legend)
...
def plot_eff(output, sigEff, bkg0Eff, bkg1Eff, savepath, filename, twopanels=True):
    ...
    canvas.SaveAs("{}{}.png".format(savepath,filename))
...
def plot_inputs(outdir, vars, branch_labels, sig, sig_w, bkg, bkg_w, bkg2, bkg2_w, sigLegend, bg0Legend, bg1Legend):
    for n, var in enumerate(vars):
        ...
        sns.distplot(sig[:, n], hist_kws={'weights': sig_w}, bins=bins, kde=False, norm_hist=True, label='{}'.format(sigLegend))
        sns.distplot(bkg[:, n], hist_kws={'weights': bkg_w}, bins=bins, kde=False, norm_hist=True, label='{}'.format(bg0Legend))
        sns.distplot(bkg2[:, n], hist_kws={'weights': bkg2_w}, bins=bins, kde=False, norm_hist=True, label='{}'.format(bg1Legend))
        #plt.title(var)
        plt.legend()
