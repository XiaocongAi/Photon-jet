...
def usage():
	print ('test usage')
	sys.stdout.write('''
			./BDT_pre.py sig bkg0 bkg1 output 
			\n''')
...
#    print ("Area under ROC curve: %.4f"%(roc_auc_score(y_test, bdt.decision_function(X_test),multi_class="ovo")))
    ...
    plot_correlations("./gbdt_results_9var/"+output+"/",  branch_names, signal, backgr)
    plot_inputs("./gbdt_results_9var/"+output+"/", branch_names, signal, None, backgr, None,signal2,None)
    joblib.dump(bdt, './gbdt_results_9var/'+output+'/'+filepath+'/bdt_model.pkl')



# Comparing train and test results
def compare_train_test(clf, X_train, y_train, X_test, y_test, output, savepath, bins=50):
    ...
    for X,y in ((X_train, y_train), (X_test, y_test)):
        ...
        #decisions += [d1, d2, d3]
        d1 = clf.decision_function(X[y==1])[:, 1]
        d2 = clf.decision_function(X[y<0.5])[:, 1]
        d3 = clf.decision_function(X[y>1.5])[:, 1]
        decisions += [d1, d2, d3]

   
    print("decisions[0]", decisions[0])
    print("decisions[1]", decisions[1])
    ...
    plt.legend(loc='best')
    plt.savefig("./gbdt_results_9var/"+output+"/"+savepath+"/BDTscore.png")
#    plt.show()
